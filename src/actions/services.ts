...
export async function getServices(options: ServiceFilterOptions | number = {}) {
  try {
    ...
    if (sortBy === "populer") {
      query = query.where(and(...whereConditions)).orderBy(desc(schema.services.viewsCount), asc(schema.services.orderIndex)) as any;
    } else {
      query = query.where(and(...whereConditions)).orderBy(desc(schema.services.createdAt), asc(schema.services.orderIndex)) as any;
    }
